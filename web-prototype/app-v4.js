 ...
 * Version: v0.4.1 (Settings ausgelagert → settings.js)
 *
 * Features:
...
(function () {
  ...
  const STORAGE_KEY_PROJECTS = 'tmm_projects'; // bleibt in app.js
  ...
  function loadSettingsFromModule() {
    if (!window.AppSettings) {
      console.warn('AppSettings nicht gefunden – verwende Defaults.');
      state.theme = 'pink';
      state.lang = 'de';
      state.mode = 'light';
      return;
  ...
  function renderProjects() {
    ...
    els.projectsEmptyHint.style.display = state.projects.length ? 'none' : 'block';
  }
  ...
  function renderFrameThumbnails() {
    ...
    project.frames.forEach((dataUrl, index) => {
      ...
      wrapper.addEventListener('click', () => selectPlayerFrame(index));
  ...
  function importData(file) {
    ...
    reader.onload = () => {
      try {
        ...
        saveProjects();
        if (window.AppSettings) {
          AppSettings.saveTheme(state.theme);
          AppSettings.saveLang(state.lang);
          AppSettings.saveMode(state.mode);
        }
  ...
  function bindEvents() {
    ...
    els.btnModeLight.addEventListener('click', () => {
      state.mode = 'light';
      if (window.AppSettings) AppSettings.saveMode(state.mode);
      applyMode();
    });
    ...
    window.addEventListener('resize', updateOverlayFromProject);
    ...
    els.settingTheme.addEventListener('change', () => {
      ...
      if (window.AppSettings) AppSettings.saveTheme(state.theme);
      applyTheme();
    });

    els.settingLang.addEventListener('change', () => {
      state.lang = els.settingLang.value === 'en' ? 'en' : 'de';
      if (window.AppSettings) AppSettings.saveLang(state.lang);
      applyLanguage();
      renderProjects();
      updateSettingsView();
    });
