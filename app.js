 ...
 *  - Optionale Anbindung von Settings.js (Mode speichern)
 * ========================================================================== */
...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (window.Library && typeof Library.init === 'function') {
      ...
      // Export-Hooks (für später)
      if (Library.setProjectExportHandlerJPEG) {
        Library.setProjectExportHandlerJPEG((projectId) => {
          alert('Export als JPEG ist noch nicht implementiert.');
        });
      }
      if (Library.setProjectExportHandlerPNG) {
        Library.setProjectExportHandlerPNG((projectId) => {
          alert('Export als PNG ist noch nicht implementiert.');
        });
      }
      if (Library.setProjectExportHandlerMP4) {
        Library.setProjectExportHandlerMP4((projectId) => {
  ...
  function bindHomeEvents() {
    ...
    if (btnHomeSettings) {
      btnHomeSettings.addEventListener('click', () => {
        alert('Einstellungen-Screen folgt später (Theme, Sprache, usw.).');
      });
    }
