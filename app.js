 ...
 *  - Overlay: letzte 3 Bilder halbtransparent auf cam_overlay
 * ========================================================================== */
...
(function () {
  ...
  const OVERLAY_MAX_FRAMES = 3; // wie viele alte Bilder als Ghost-Overlay
  ...
  function showTab(name) {
    ...
    // Wenn wir in die Kamera wechseln: Overlay aktualisieren
    if (name === 'camera') {
      updateOverlay();
    }
  ...
  async function startCamera() {
    ...
    try {
      ...
      // wenn Kamera läuft und Projekt vorhanden → Overlay aufbauen
      updateOverlay();
    } catch (err) {
  ...
  function onCaptureClick() {
    ...
    // Overlay mit letzten Bildern aktualisieren
    updateOverlay();
  }

  /* -----------------------------------------------------------------------
   * Overlay (letzte Bilder als Ghost)
   * -------------------------------------------------------------------- */
  ...
  function clearOverlay() {
    ...
    if (!ctx) return;
    camOverlay.width  = camOverlay.clientWidth  || 0;
    camOverlay.height = camOverlay.clientHeight || 0;
  ...
  function updateOverlay() {
    ...
    const frames = currentProject.frames.slice(-OVERLAY_MAX_FRAMES);

    const ctx = camOverlay.getContext('2d');
    if (!ctx) return;

    const w = camOverlay.width  = camOverlay.clientWidth  || 0;
    const h = camOverlay.height = camOverlay.clientHeight || 0;
    ...
    frames.forEach((dataUrl, index) => {
      ...
      img.onload = () => {
        ...
        if (loaded === frames.length) {
          // alle Bilder geladen → zeichnen
          ctx.clearRect(0, 0, w, h);
          images.forEach((im, i) => {
            if (!im) return;
            ctx.save();
            // ältere Bilder sind transparenter
            const baseAlpha = 0.18;
            const step = 0.12;
            ctx.globalAlpha = baseAlpha + step * i;
  ...
  function openProject(project) {
    ...
    // Overlay mit vorhandenen Frames
    updateOverlay();
